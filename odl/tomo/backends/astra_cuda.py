...
class AstraCudaImpl:
    ...
    def _call_backward_kats_real(self, x, out=None, **kwargs):
        ...
        with self._mutex:
            ...
            scale_coeff = astra_bp_scaling * projs_per_turn # why this value?
                                                            # can you bring it to one of the
                                                            # original values given in metadata?
...
def astra_cuda_bp_scaling_factor(proj_space, vol_space, geometry, angle_weighting=None):
    ...
    extent_arr = proj_space.partition.extent
    if angle_weighting:
        extent_arr[0] = angle_weighting
    proj_extent = float(extent_arr.prod())
