...
class KatsevichFilterCurved(Operator):
    def __init__(self, ray_trafo):
        ...
        if self.const_pitch:
            ...
            for i, a in enumerate(self.alpha_vals):
                cos_a = np.cos(a)
                sin_a = np.sin(a)
                for k, angle in enumerate(self.geometry.angles):
                    self.fwd_rebin_w[k, i] = ((self.D * (self.P + self.geometry.src_shift_func(angle)[:, 2]) / (2 * np.pi * self.Rs)) * (
                        self.psi_vals * cos_a + self.psi_vals / tan_psi * sin_a))
        ...
        if self.const_pitch:
            ...
            for k in tqdm(range(self.N_s), desc='Precomputing rebinning rows -> detector rows (nonconst P)'):
                ...
                for row in range(self.N_w):
                    ...
                    for col in range(self.N_alpha//2, self.N_alpha):
                        self.rebin_row[k, col, row] = 0
                        for rebin in range(self.detector_rebin_rows - 1):
                            if (self.w_vals[row] >= self.fwd_rebin_w[k, col, rebin]) \
                                and (self.w_vals[row] <= self.fwd_rebin_w[k, col, rebin+1]):
                                self.rebin_row[k, col, row] = rebin
                                break
                        self.rebin_fracs_0[k, col, row] = (self.w_vals[row] - self.fwd_rebin_w[k, col, self.rebin_row[k, col, row]]) \
                            / (self.fwd_rebin_w[k, col, self.rebin_row[k, col, row] + 1] - self.fwd_rebin_w[k, col, self.rebin_row[k, col, row]])

                    for col in range(self.N_alpha//2):
                        self.rebin_row[k, col, row] = 1
                        for rebin in range(self.detector_rebin_rows - 1, 0, -1):
                            if (self.w_vals[row] >= self.fwd_rebin_w[k, col, rebin - 1]) \
                                and (self.w_vals[row] <= self.fwd_rebin_w[k, col, rebin]):
                                    self.rebin_row[k, col, row] = rebin
                                    break
                        self.rebin_fracs_0[k, col, row] = (self.w_vals[row] - self.fwd_rebin_w[k, col, self.rebin_row[k, col, row] - 1]) \
                            / (self.fwd_rebin_w[k, col, self.rebin_row[k, col, row]] - self.fwd_rebin_w[k, col, self.rebin_row[k, col, row] - 1])
    ...
    def _call(self, g, out=None, **kwargs):
        ...
        if self.const_pitch:
            ...
            self._g3 = self.cf3_forward_rebin_nonconst_pitch(self._g2)
        ...
        if self.const_pitch:
            ...
            self._g5 = self.cf5_backward_rebin_nonconst_pitch(self._g4)
    ...
    def cf3_forward_rebin_nonconst_pitch(self, g2):
    ...
    def cf5_backward_rebin_nonconst_pitch(self, g4):
        # reverse the rebinning process. Use the precomputation 
        # of the weights c, 1-c and of the map rebinning rows -> detector rows.
        # see formula (55) Noo et al. 2003.
        g5 = np.zeros((self.N_s, self.N_alpha, self.N_w), dtype=g4.dtype)
